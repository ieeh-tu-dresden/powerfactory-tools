...
class Case(Base):
    ...
    def _is_proper_element_number(self, topology: Topology) -> bool:
        if len(self.loads) != len(topology.loads):
            logger.error(
                "Number of loads does not match. Is %i, should be %i.",
                len(self.loads),
                len(topology.loads),
            )
            return False
        ...
        if len(self.transformers) != len(topology.transformers):
            logger.error(
                "Number of transformers does not match. Is %i, should be %i.",
                len(self.transformers),
                len(topology.transformers),
            )
            return False

        if len(self.external_grids) != len(topology.external_grids):
            logger.error(
                "Number of external grids does not match. Is %i, should be %i.",
                len(self.external_grids),
                len(topology.external_grids),
            )
            return False
    ...
    def _is_proper_loads(self, topology: Topology) -> bool:
        ...
        for load in topology.loads:
            if load.name not in load_names:
                logger.error("Load %s is not in steadystate case.", load.name)
                return False
    ...
    def _is_proper_transformers(self, topology: Topology) -> bool:
        ...
        for trafo in topology.transformers:
            if trafo.name not in transformer_names:
                logger.error("Transformer %s is not in steadystate case.", trafo.name)
                return False
    ...
    def _is_proper_external_grids(self, topology: Topology) -> bool:
        ...
        for ext_grid in topology.external_grids:
            if ext_grid.name not in external_grid_names:
                logger.error("External grid %s is not in steadystate case.", ext_grid.name)
                return False
